pipeline {
  ...
  parameters {
    ...
    string(name: 'ApplicationScope', defaultValue: 'Product Catalog', description: 'Comma-separated list of LifeTime applications to deploy.')
    string(name: 'ApplicationScopeWithTests', defaultValue: 'Product Catalog , BDDProductCatalog', description: 'Comma-separated list of LifeTime applications to deploy (including test applications)')
  ...
  environment {
    ...
    */
    ...
    ProbeEnvironmentURL = 'https://qa.graodegente.com.br/'
    BddEnvironmentURL = 'https://qa.graodegente.com.br/'
  ...
  stages {
    ...
    stage('Install Requirements') {
      steps {
        ...
        withPythonEnv('python') {
          echo "Install Python requirements"
          bat 'pip install  -r C:/Users/TI/Desktop/copia_selenium/requirements.txt'
        }
      }
    }
    stage('Run Selenium') {
      steps {
        withPythonEnv('python') {
          // Generate the URL endpoints of the BDD tests
          powershell "python C:/Users/TI/Desktop/copia_selenium/src/gerador_resultado.py"
        }
      }
    }
    stage('Deploy Production') {
      steps {
        ...
        withPythonEnv('python') {
          ...
          powershell "python -m outsystems.pipeline.deploy_latest_tags_to_target_env --artifacts \"${env.ArtifactsFolder}\" --lt_url ${env.LifeTimeHostname} --lt_token ${env.AuthorizationToken} --lt_api_version ${env.LifeTimeAPIVersion} --source_env \"${env.RegressionEnvironment}\" --destination_env \"${env.ProductionEnvironment}\" --app_list \"${params.ApplicationScope}\""
        }
        }      }
      post {
        always {
